...
from torchrl.record.loggers import generate_exp_name  # noqa: F401 – kept for restore compatibility
...
class Experiment(CallbackNotifier):
    ...
    def _perform_checks(self):
        pass  # model checks removed (only MLP supported)
    ...
    def _build_wandb_run_name(self) -> str:
        ...
        if demir_scale > 0:
            ...
            suffix = (
                f"demir_scale{fmt(demir_scale)}"
                f"_encoder_{enc}"
                f"_beta1_{fmt(beta1)}_beta2_{fmt(beta2)}"
            )
        elif rnd_scale > 0:
            suffix = f"rnd_scale{fmt(rnd_scale)}"
        elif ngu_scale > 0:
            suffix = f"ngu_scale{fmt(ngu_scale)}_encoder_idm"
        else:
            suffix = None
    ...
    def _setup_name(self):
        ...
        if self.config.restore_file is None:
            self.name = self._build_wandb_run_name()
    ...
    def _collection_loop(self):
        ...
        for _ in range(
            ...
            print(
                f"[iter {self.n_iters_performed:>4d} | "
                f"frames {self.total_frames:>9,} / {self.config.max_n_frames:,}] "
